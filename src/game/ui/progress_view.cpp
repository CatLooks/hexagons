...
namespace gameui {
	...
	void Progress::reconstruct(const std::vector<Region::Team>& teams) {
		...
		for (size_t i = 0; i < teams.size(); i++) {
			...
			switch (i + 1) {
				case 1: arr[0]->setColor(sf::Color(0xef, 0xbf, 0x04)); break; // gold
				case 2: arr[0]->setColor(sf::Color(0xc4, 0xc4, 0xc4)); break; // silver
				case 3: arr[0]->setColor(sf::Color(0xce, 0x89, 0x46)); break; // bronze
				default: break;
			};
	...
	/// Updates table data.
	void Progress::update(const logic::TileCount& count, const std::vector<Messages::Player>& players) {
		/// Label order.
