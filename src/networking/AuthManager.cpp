...
void AuthManager::HandleEpicLoginComplete(const EOS_Connect_LoginCallbackInfo* Data) {
    if (Data->ResultCode == EOS_EResult::EOS_Success)
    {
		...
		EOS_UserInfo_BestDisplayName* DisplayName = nullptr;
		...
		options.LocalUserId = Manager->m_EpicId;
		options.TargetUserId = Manager->m_EpicId;
        ...
        if (result == EOS_EResult::EOS_Success && DisplayName) {
			...
			std::cerr << "Failed to get user display name: " << EOS_EResult_ToString(result) << std::endl;
        }

        // Announce to the world that we have logged in successfully!
        // Any code that is "listening" will now be executed.
        Manager->OnLoginSuccess.invoke(Manager->m_LocalUserId);
    }
