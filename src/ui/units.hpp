...
namespace ui {
	...
	struct Dim {
		...
		Dim operator-(const Dim& oth) const { return { px - oth.px, ps - oth.ps, es - oth.es }; };
		/// Scale the dimension by a factor.
		Dim operator*(float scale) const { return { (int)(px * scale), ps * scale, es * scale }; };
	...
	struct DimVector {
		...
		DimVector(sf::Vector2i vec) : x(vec.x), y(vec.y) {};
	...
	struct DimRect {
		...
		DimRect(sf::IntRect rect) : DimRect(rect.position.x, rect.position.y, rect.size.x, rect.size.y) {};
	};
};
